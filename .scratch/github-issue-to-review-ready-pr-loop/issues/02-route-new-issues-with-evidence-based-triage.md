...
- [x] Malformed JSON, an unrecognized label, a missing or empty comment, or an unavailable model fails closed without changing the issue.
...
- YAML parsing, `actionlint`, OpenCode agent and skill discovery, JSONL extraction, both type-checks,
  and all 84 repository tests pass. A controlled default-branch run proved the trigger, read-only
  permissions, and decision extraction. The remaining acceptance step is proving label and comment
  application after the four readiness labels are configured.
