...
- Triage has no dedicated configuration preflight. A missing model credential, unavailable model,
  malformed result, or failed analysis stops the read-only job, so the apply job never changes the
  issue.
...
- Agent jobs receive repository and issue read access plus the OpenCode credential, but no
  write-capable GitHub token. Deterministic mutation and publication jobs request built-in token
  permissions only after agent work and required checks complete.
- Read-only agent work and write-capable GitHub mutations use separate jobs so model execution
  does not share the mutation credential's runner environment.
...
- The repository-side implementation can proceed before credentials are supplied. Live workflows
  ...
  labels and `OPENCODE_API_KEY` are present.
