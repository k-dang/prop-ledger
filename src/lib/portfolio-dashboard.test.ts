...
describe("portfolio dashboard summary", () => {
  ...
  it("adds non-rent manual income to rent-ledger income", () => {
    const parkingIncome = makeEntry("parking-income-1", {
      type: "income",
      date: "2026-01-02",
      amount: 150,
      expenseCategory: null,
    ...
    expect(summary.totals.grossRentalIncome).toBe(2550);
    expect(summary.properties[0]?.grossRentalIncome).toBe(2550);
  });
